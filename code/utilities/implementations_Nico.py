...
def compute_logistic_loss(y, tx, w, lambda_ = 0):
	...
	return -np.sum(y * np.log(sigmoid(tx, w)) + (1 - y) * np.log(1 - sigmoid(tx, w))) / y.shape[0] + lambda_ * w[np.newaxis,:].dot(w[:,np.newaxis])
...
def ridge_regression(y, tx, lambda_):
    """ Ridge regression (OLS with L2-regularization) using normal equations """
    w = np.linalg.solve(tx.T.dot(tx) + lambda_ * (2. * y.shape[0]) * np.eye(tx.shape[1]), tx.T.dot(y))
    loss = compute_mse_loss(y, tx, w) + lambda_ * np.linalg.norm(w)**2
    return w, loss
